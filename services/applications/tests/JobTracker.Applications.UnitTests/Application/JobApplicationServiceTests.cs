...
public class JobApplicationServiceTests
{
    ...
    public async Task CreateAsync_persists_and_returns_mapped_response()
    {
        ...
        await _repository.Received(1).AddAsync(
            Arg.Is<JobApplication>(a => a.Company == "Acme"), Arg.Any<CancellationToken>());
    ...
    public async Task CreateAsync_throws_and_does_not_persist_when_invalid()
    {
        ...
        await Assert.ThrowsAsync<ValidationException>(() => _sut.CreateAsync(invalid));
    ...
    {
        var existing = JobApplication.Create("Acme", "Engineer", ApplicationStatus.Applied, ApplicationSource.LinkedIn);
